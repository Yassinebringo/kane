...
bot.on("ready", () => {
  ...
  bot.user.setActivity('!help to view commands');
});
...
> \`play\` > **\`play [title/url]\`**
> \`search\` > **\`search [title]\`**
> \`skip\`, \`disconnect\`,  \`pause\`, \`resume\`
> \`nowplaying\`, \`queue\`, \`volume\`, \`lyrics\``);
    ...
    if (command === "search" || command === "sc") {
    ...
    } else if (command === "disconnect" || command === "dc" || command === "fuckoff"  ) {
        ...
        serverQueue.songs = [];
        serverQueue.connection.dispatcher.end("[runCmd] Disconnect command has been used");
        ...
        serverQueue.connection.dispatcher.setVolume(args[1] / 75);
      ...
      if(!args[1]) {
        const errorEmbed = new MessageEmbed()
          .setColor('#ff0000')
          .setDescription('Proper command usage: `lyrics` `song name`');
        return message.channel.send(errorEmbed);
      }
      
      args.shift()
      const songResponse = await fetch(`https://api.genius.com/search?q=${args.join('%20')}&access_token=${process.env.GENIUS}`);
      const songData = await songResponse.json();
      if(!songData.response.hits[0]) {
        const errorEmbed = new MessageEmbed()
          .setColor('#ff0000')
          .setDescription(`Couldn't find any lyrics for ${args.join(' ')}`);
        return message.channel.send(errorEmbed);
      }
      
      const lyricsEmbed = new MessageEmbed()
        .setColor('GREEN')
        .setTitle(`📃 **|** ${songData.response.hits[0].result.full_title}`)
        .setDescription(songData.response.hits[0].result.url);
...
function play(guild, song) {
    ...
    dispatcher.setVolume(serverQueue.volume / 75);
